...
class TransactionService {
    ...
    calculateReorderStatus(quantity) {
        return quantity <= 10 ? "BELOW_REORDER" : "OK";
    }
    // MAIN SEARCH METHOD with pagination
    searchTransactionsService = async (params) => {
    ...
    searchTransactionsByWarehouseService = async (warehouseId, params) => {
    ...
    getTransactionByIdService = async (id) => {
        return await db_1.prisma.transaction.findUnique({
            where: { id },
            include: {
                warehouse: true,
                performedBy: {
    ...
    createTransactionService = async (data) => {
        return await db_1.prisma.$transaction(async (tx) => {
            const { type, warehouseId, performedById, referenceId, referenceType, notes, items, } = data;
            // Validate warehouse
    ...
    updateTransactionService = async (id, data) => {
        return await db_1.prisma.transaction.update({
            where: { id },
            data: {
                notes: data.notes,
            },
    ...
    getTransactionStatisticsService = async () => {
        ...
        const [totalTransactions, todayTransactions, monthlyTransactions, yearlyTransactions, transactionsByType,] = await Promise.all([
