...
class OrderService {
    ...
    searchOrdersService = async (params) => {
        ...
        const ordersWithMetrics = await Promise.all(data.map(async (order) => {
            ...
            const itemsWithCarbon = await Promise.all(order.items.map(async (item) => {
            ...
            const totalCarbon = itemsWithCarbon.reduce((sum, item) => sum + item.carbonImpact, 0);
            return {
                ...order,
    ...
    searchOrdersByStatusService = async (status, params) => {
    ...
    searchOrdersToFulfillService = async (params) => {
        return this.searchOrdersService({
            ...params,
            filters: {
                ...params.filters,
    ...
    getOrderByIdService = async (id) => {
        ...
        const itemsWithAvailability = await Promise.all(order.items.map(async (item) => {
            const totalAvailable = item.product.inventory.reduce((sum, inv) => sum + inv.available, 0);
    ...
    // UPDATE order
    updateOrderService = async (id, data) => {
        return await db_1.prisma.$transaction(async (tx) => {
            ...
            if (data.status) {
                ...
                if (!allowedNextStatuses.includes(data.status)) {
                    throw new Error(`Invalid status transition from ${existingOrder.status} to ${data.status}`);
                }
    ...
    getOrderStatisticsService = async () => {
        ...
        const [totalOrders, todayOrders, monthlyOrders, yearlyOrders, ordersByStatus, ordersByPriority, totalRevenue, averageOrderValue,] = await Promise.all([
        ...
        // Get top customers
        const topCustomers = await db_1.prisma.order.groupBy({
            by: ["customerName"],
            _count: { id: true },
            _sum: { totalAmount: true },
    ...
    calculateFulfillmentProgress = async (orderId) => {
        ...
        const totalWeight = items.reduce((sum, item) => sum + (statusWeights[item.status] || 0), 0);
        return Math.round(totalWeight / items.length);
    };
    // Helper: Get fulfillment warehouses for order
    getFulfillmentWarehousesForOrder = async (orderId) => {
        const order = await db_1.prisma.order.findUnique({
            where: { id: orderId },
