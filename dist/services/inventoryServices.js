...
class InventoryServices {
    ...
    searchInventoryService = async (params) => {
        ...
        if (filters.lowStockOnly) {
            ...
            const productMinStockMap = new Map(products.map(p => [p.id, p.minStockLevel]));
    ...
    searchLowStockInventoryService = async (params) => {
    ...
    searchInventoryByProductService = async (productId, params) => {
        return this.searchInventoryService({
            ...params,
    ...
    getInventoryByIdService = async (id) => {
        return await db_1.prisma.inventory.findUnique({
            where: { id },
            include: {
                warehouse: true,
                product: {
    ...
    upsertInventoryService = async (data) => {
        ...
        const [warehouse, product] = await Promise.all([
            db_1.prisma.warehouse.findUnique({ where: { id: warehouseId } }),
            db_1.prisma.product.findUnique({ where: { id: productId } }),
        ]);
        if (!warehouse)
        ...
        return await db_1.prisma.inventory.upsert({
            where: {
    ...
    updateInventoryQuantityService = async (id, quantity, action = "SET", notes) => {
        ...
        return await db_1.prisma.inventory.update({
            where: { id },
            data: {
                quantity: newQuantity,
                available: Math.max(0, newQuantity - inventory.reserved),
                lastUpdated: new Date(),
                reorderStatus: this.calculateReorderStatus(newQuantity, inventory.product.minStockLevel, inventory.product.reorderPoint),
            },
            include: {
                warehouse: true,
                product: true,
    ...
    deleteInventoryService = async (id) => {
        ...
        return await db_1.prisma.inventory.delete({
            where: { id },
        });
    };
    // Bulk update inventory
    bulkUpdateInventoryService = async (updates) => {
        const results = [];
        for (const update of updates) {
            try {
                const result = await this.updateInventoryQuantityService(update.id, update.quantity, update.action, update.notes);
    ...
    getInventorySummaryService = async () => {
        const [totalItems, totalQuantity, outOfStockCount, warehousesWithStock,] = await Promise.all([
            db_1.prisma.inventory.count(),
            db_1.prisma.inventory.aggregate({
                _sum: { quantity: true },
            }),
            db_1.prisma.inventory.count({ where: { quantity: 0 } }),
            db_1.prisma.warehouse.count({
                where: {
                    inventory: {
                        some: { quantity: { gt: 0 } },
                    },
    ...
    getInventoryValueReportService = async (filters) => {
        const where = {};
        if (filters.warehouseId)
            where.warehouseId = filters.warehouseId;
        if (filters.categoryId)
        ...
        let inventory = await db_1.prisma.inventory.findMany({
            where,
            include: {
                warehouse: true,
                product: {
    ...
    calculateReorderStatus = (quantity, minStockLevel, reorderPoint) => {
    ...
    calculateTotalInventoryValue = async () => {
    ...
    getLowStockCount = async () => {
        ...
        const allInventory = await db_1.prisma.inventory.findMany({
            where: {
...
exports.default = InventoryServices;
