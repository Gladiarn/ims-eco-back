...
class WarehouseServices {
    ...
    searchWarehousesService = async (params) => {
        ...
        const [data, total] = await Promise.all([
            ...
            db_1.prisma.warehouse.count({ where }),
        ]);
        return {
            data,
            pagination: {
    ...
    getAllWarehousesService = async () => {
        return await db_1.prisma.warehouse.findMany({
            include: {
                manager: {
                    select: {
                        id: true,
    ...
    // GET warehouse by ID
    getWarehouseByIdService = async (id) => {
        return await db_1.prisma.warehouse.findUnique({
            where: { id },
            include: {
                manager: true,
                inventory: {
    ...
    // CREATE warehouse
    createWarehouseService = async (data) => {
        ...
        const existing = await db_1.prisma.warehouse.findUnique({
            where: { code: data.code },
        });
        if (existing) {
            throw new Error("Warehouse code already exists");
        }
        return await db_1.prisma.warehouse.create({
            data: {
                ...data,
    ...
    // UPDATE warehouse
    updateWarehouseService = async (id, data) => {
        return await db_1.prisma.warehouse.update({
            where: { id },
            data,
        });
    };
    // DELETE warehouse
    deleteWarehouseService = async (id) => {
        // Check if warehouse has inventory
        const inventoryCount = await db_1.prisma.inventory.count({
            where: { warehouseId: id },
        });
        if (inventoryCount > 0) {
            throw new Error("Cannot delete warehouse with inventory. Transfer or remove inventory first.");
        }
        return await db_1.prisma.warehouse.delete({
            where: { id },
        });
    };
    // GET warehouse inventory summary
    getWarehouseInventorySummaryService = async (id) => {
        ...
        return {
            ...
            lowStockItems: inventory.filter((item) => item.quantity <= item.product.minStockLevel).length,
            outOfStockItems: inventory.filter((item) => item.quantity === 0).length,
        };
    };
    // GET warehouse statistics
    getWarehouseStatsService = async (id) => {
        const [inventory, transfers, stockCounts] = await Promise.all([
            db_1.prisma.inventory.count({ where: { warehouseId: id } }),
            db_1.prisma.transfer.count({
                where: {
                    OR: [{ sourceWarehouseId: id }, { destWarehouseId: id }],
                },
            }),
            db_1.prisma.stockCount.count({ where: { warehouseId: id } }),
        ]);
        return {
    ...
    searchWarehousesWithLowStockService = async (params) => {
        ...
        const allWarehouses = await db_1.prisma.warehouse.findMany({
            where,
            include: {
                manager: {
                    select: {
...
exports.default = WarehouseServices;
